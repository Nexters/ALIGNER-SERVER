...
interface TargetPoseQueryService {
    ...
    fun getAllByBodyPartCode(bodyPartCode: String): List<TargetPoseSummaryView>
}
...
internal class TargetPoseQueryServiceImpl(
    ...
    /**
     * 해당 부위에 자세가 없으면 빈 목록이 정상이다. 예외를 던지지 않는다.
     *
     * bodyPartCode 는 screening 소유 어휘라 catalog 가 값 집합을 검증하지 않는다. 모르는 코드가
     * 들어오면 빈 목록이 나간다 (docs/domains.md §4-2).
     */
    override fun getAllByBodyPartCode(bodyPartCode: String): List<TargetPoseSummaryView> =
        targetPoseQueryRepository.findAllByBodyPartCode(bodyPartCode)
}
