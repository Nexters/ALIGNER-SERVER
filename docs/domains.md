...
제안할 뿐이고, 그 뒤 「자세 도전 현황」이 핀포즈 전체를 펼치면 회원은 진단에 없던 부위도 눌러
시작한다. 그래서 `course`는 최신 원인에서 부위가 맞는 것을 찾아 `cause_code`에 남기되,
없다고 거절하지 않는다. 진단을 **한 번도 하지 않은** 회원만 막는다(`SCREENING_REQUIRED`) —
그건 부위 선택의 문제가 아니라 온보딩을 건너뛴 것이다.
...
9. ~~**오늘 코스와 내일 코스의 차이.**~~ **해소 — 내일의 코스가 없다.** 살아 있는 화면에
   ...
   **「내일 운동 미리보기」가 이 결정을 뒤집지 않는다.** 완료 상태 홈에 붙는 이 카드는
   예약이 아니라 **제안**이다 — 같은 부위에서 아직 4 번 완수하지 못한 자세 중 하나를
   조회 시점에 무작위로 고른다. 서버가 내일 할 코스를 정해 저장하지 않으므로 `scheduled_on`은
   여전히 없다. 다만 난수의 씨앗을 (회원, 날짜)로 고정해 **하루 안에서는 같은 자세**가 나온다 —
   홈을 다시 불러올 때마다 카드가 바뀌면 회원이 이것을 제안으로도 읽지 못한다. 카드를 누르면
   `POST /courses`를 호출한다 — 추천은 멱등하고 완주한 코스는 그때 다음 회차로 다시 열린다.

   다만 **"오늘의 코스"의 범위가 넓어졌다.** 완주하는 순간 `status`가 `COMPLETED`가 되는데
   거기서 홈이 404를 받으면 완료 상태 화면을 그릴 수 없다. `GET /courses/today`는 진행 중인
   코스를 우선하되, 없으면 **오늘(Asia/Seoul) 완주한 코스**를 돌려준다. 어제 완주한 코스는
   오늘 할 일이 아니라 지난 기록이라 끌고 오지 않는다.
