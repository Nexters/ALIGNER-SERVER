...
#### 예외 — `application-mock` (한시적)

**`application-mock`이 두 번째 Spring Boot 실행 모듈이다.** 단일 배포 단위 결정의 예외이므로
근거와 삭제 조건을 여기 적어 둔다.

- **왜 있나.** 감수 콘텐츠 seed가 없어 실제 서버로는 로그인 다음 화면부터 아무것도 그릴 수
  없다. 프론트가 화면과 화면 전환을 실제 HTTP로 만들 수 있게 하는 임시 도구다
- **왜 `application-api`의 프로파일이 아닌가.** 더미 코드가 운영 아티팩트에 실리고, 프로파일
  오설정 시 **운영이 조용히 가짜 데이터를 내보낸다.** `CorsProperties`가 "설정을 빠뜨리면
  닫히는 쪽으로 실패해야 한다"고 정한 것과 반대 방향이다. 별도 모듈이면 목 코드가 운영
  jar에 아예 없어 그 사고가 불가능하다
- **배포 대상이 아니다.** `bootJar`를 꺼두었으므로 실행 가능한 산출물이 만들어지지 않는다.
  파이프라인 설정이 아니라 **빌드가 그것을 강제한다.** 개발자는 `bootRun`으로만 띄운다
- **언제 지우나.** 감수 seed와 `catalog/adapter-ymove`가 들어오면 **모듈째로 지운다.**
  그때 이 절도 함께 지운다

이 예외는 여기 적힌 조건 안에서만 유효하다. 다른 실행 모듈을 추가하려면 §1의 단일 배포 단위
결정을 먼저 다시 정한다.
